...
class homeController extends Controller
{
    ...
    ->first();
        ...
        $rekomendasi = artikelModel::with('getKategori', 'creator')->where('status', true)->whereNotIn('id', [$artikel->id])->inRandomOrder()->take(6)->get();
    ...
    public function sendkontak(Request $request)
    {
        ...
        Mail::send([], [], function (Message $message) use ($validatedData) {
            $message->to('pelayanan@mekarmukti.id')
                ->subject($validatedData['subject'])
                ->from($validatedData['email'], $validatedData['name'])
                ->html($validatedData['message']);
        });

        return back()->with('pesan', 'Pesan Kamu Telah Terkirim. Terima Kasih!');
    }
}
