...
<div class="page-header">
    <div class="page-header-content">
        <div>
            <h1 class="page-title">📚 Kelola Majalah</h1>
            <p class="page-subtitle">Kelola cover majalah digital desa dengan interface yang modern dan user-friendly</p>
        </div>
...
<div class="stats-grid mb-4">
    <div class="stats-card">
        ...
        <div class="stats-value">{{ $majalah->count() }}</div>
        <div class="stats-label">Total Majalah</div>
    </div>
    ...
    <div class="stats-card">
        ...
        <div class="stats-value">{{ $majalah->groupBy('type')->count() }}</div>
    ...
    <div class="stats-card">
        ...
        <div class="stats-value">{{ $majalah->where('created_at', '>=', now()->subDays(7))->count() }}</div>
        <div class="stats-label">Majalah Minggu Ini</div>
    </div>
        ...
        <div class="row align-items-center">
            <div class="col-md-4 mb-3 mb-md-0">
                <div class="input-group">
                    ...
                    <input type="text" class="form-control border-start-0" placeholder="Cari majalah..." id="searchInput">
                </div>
            </div>
...
<div class="card">
    <div class="card-header d-flex align-items-center justify-content-between">
        ...
        <div class="d-flex align-items-center gap-3">
            <small class="text-muted" id="photoCount">{{ $majalah->count() }} majalah</small>
            <div class="dropdown">
    ...
    <div class="card-body p-0">
        <div id="galleryContainer" class="gallery-list">
                ...
                <div class="gallery-item-content">
                    ...
                    <div class="gallery-thumbnail">
                        <div class="image-container">
                            @if($item->cover_image && file_exists(public_path('majalah/' . basename($item->cover_image))))
                            <img src="{{ asset('majalah/' . basename($item->cover_image)) }}" alt="{{ $item->judul }}" class="gallery-image"
                                 onclick="openImageModal('{{ asset('majalah/' . basename($item->cover_image)) }}', '{{ $item->judul }}')">
                            @elseif($item->cover_image && file_exists(public_path('storage/' . basename($item->cover_image))))
                            <img src="{{ asset('storage/' . basename($item->cover_image)) }}" alt="{{ $item->judul }}" class="gallery-image"
                                 onclick="openImageModal('{{ asset('storage/' . basename($item->cover_image)) }}', '{{ $item->judul }}')">
                            @elseif($item->cover_image && file_exists(public_path('galeri/' . basename($item->cover_image))))
                            <img src="{{ asset('galeri/' . basename($item->cover_image)) }}" alt="{{ $item->judul }}" class="gallery-image"
                                 onclick="openImageModal('{{ asset('galeri/' . basename($item->cover_image)) }}', '{{ $item->judul }}')">
                            @elseif($item->cover_image)
                            <img src="{{ asset($item->cover_image) }}" alt="{{ $item->judul }}" class="gallery-image"
                                 onclick="openImageModal('{{ asset($item->cover_image) }}', '{{ $item->judul }}')"
                                 onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                            <div class="no-image d-flex align-items-center justify-content-center bg-light" style="height: 100%; display: none;">
                            ...
                            @if($item->cover_image && (file_exists(public_path('majalah/' . basename($item->cover_image))) || file_exists(public_path('storage/' . basename($item->cover_image))) || file_exists(public_path('galeri/' . basename($item->cover_image))) || file_exists(public_path($item->cover_image))))
                            <div class="image-overlay">
                                <button class="btn btn-light btn-sm" onclick="openImageModal('{{ 
                                    file_exists(public_path('majalah/' . basename($item->cover_image))) ? asset('majalah/' . basename($item->cover_image)) :
                                    (file_exists(public_path('storage/' . basename($item->cover_image))) ? asset('storage/' . basename($item->cover_image)) : 
                                    (file_exists(public_path('galeri/' . basename($item->cover_image))) ? asset('galeri/' . basename($item->cover_image)) : asset($item->cover_image)))
                                }}', '{{ $item->judul }}')">
                    ...
                    <div class="gallery-info">
                        <div class="gallery-details">
                            ...
                            <div class="gallery-meta">
                                @if($item->type)
                                <span class="badge badge-primary mb-2">{{ $item->type }}</span>
                                @endif
                                <div class="d-flex flex-wrap gap-2 text-sm text-muted">
                                    ...
                                    <span>
                                        ...
                                        {{ strtoupper(pathinfo($item->cover_image, PATHINFO_EXTENSION)) }}
                                    </span>
            ...
            <div class="text-center py-5">
                ...
                <h4 class="text-muted mb-2">Majalah masih kosong</h4>
                <p class="text-muted mb-4">Mulai upload cover majalah pertama untuk koleksi digital desa Anda</p>
                <button onclick="openUploadModal(true)" class="btn btn-primary">
                    <i class="fas fa-plus me-2"></i>
                    Upload Majalah Pertama
                </button>
            </div>
            @endforelse
        </div>
        ...
        <div id="noResults" class="text-center py-5" style="display: none;">
            ...
            <h4 class="text-muted mb-2">Tidak ada majalah yang cocok</h4>
    ...
    <div class="card-footer bg-light">
        <div class="d-flex justify-content-between align-items-center">
            <span class="text-muted">
                <span id="visibleCount">{{ $majalah->count() }}</span> dari {{ $majalah->count() }} majalah
            </span>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="uploadModalLabel">
                    ...
                    Upload Cover Majalah
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="uploadForm" action="{{ route('majalah.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <!-- Title Input -->
                            <div class="mb-3">
                                <label for="galleryTitle" class="form-label">Judul Majalah</label>
                                <input type="text" class="form-control" id="galleryTitle" name="judul" placeholder="Masukkan judul majalah...">
                            </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="editForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="editTitle" class="form-label">Judul Majalah</label>
                        <input type="text" class="form-control" id="editTitle" name="judul" required>
                    </div>
                    <div class="mb-3">
                        <label for="editImage" class="form-label">Ganti Cover (Opsional)</label>
...
<form id="deleteForm{{ $item->id }}" action="{{ route('majalah.destroy', $item->id) }}" method="POST" class="d-none">
    @csrf
    @method('DELETE')
</form>
@endforeach
...
<script>
    ...
    function deleteSelected() {
        ...
        if (selected.length === 0) {
            showToast('Pilih majalah yang ingin dihapus terlebih dahulu', 'warning');
            return;
        }

        if (confirm(`Apakah Anda yakin ingin menghapus ${selected.length} majalah yang dipilih?\n\nTindakan ini tidak dapat dibatalkan.`)) {
            showLoading();
            // Submit form for bulk delete
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = '{{ route("majalah.bulk-delete") }}';
            form.innerHTML = `
                @csrf
                @method('DELETE')
    ...
    function editGalleryItem(id, title) {
        ...
        document.getElementById('editForm').action = `/admin/majalah/${id}`;
        document.getElementById('editTitle').value = title;
        modal.show();
    }

    function deleteGalleryItem(id, title) {
        if (confirm(`Apakah Anda yakin ingin menghapus majalah "${title}"?\n\nTindakan ini tidak dapat dibatalkan.`)) {
            showLoading();
